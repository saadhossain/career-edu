...
const Register = () => {
    const { userRegister, emailVerify, updateUserProfile, googleLogin} = useContext(AuthContext);
    ...
    const handleRegister = (e) => {
        ...
        userRegister(email, password)
            .then(result => {
                ...
                emailVerify()
                    .then(() => {
                        toast.success('A Verificaton email has been sent to your email, please verify')
                    })
    ...
    //Handle User Registration using google
        ...
        <div className='flex justify-center mt-10'>
            <div className="w-full max-w-md p-8 space-y-3 rounded-xl bg-eduhf text-white">
                ...
                <p className="text-md text-center sm:px-6">Already have an account?
                    <Link rel="noopener noreferrer" to='/login' className="underline"> Login</Link>
                </p>
            </div>
        </div>
    );
